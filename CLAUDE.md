...
**Routing**: no router library. `RootRoute` gates on auth (`Spin` while loading, `LoginPage` if signed out, otherwise renders children) and `AppRoute` renders the tabbed category UI (`CategoryTabs`, one `ItemList` per category plus a Settings tab). A second real route (e.g. a standalone Settings page) would be the trigger to introduce TanStack Router — it's deliberately not installed yet.
...
- Known flaky test: `src/features/categories/useCategories.test.tsx`'s "does not duplicate defaults" test intermittently fails under vitest's default parallel file execution, because multiple test files share one Firestore emulator instance and `clearFirestoreEmulator` wipes the whole thing — one file's wipe can land mid another file's test. Reproduces reliably clean under `--no-file-parallelism`. Consider setting `fileParallelism: false` in `vitest.config.ts` if this becomes disruptive.
...
`src/routes/app-route.tsx` calls its own independent `useAuth()` instead of receiving `user` as a prop from `RootRoute` (which already resolved it before rendering `AppRoute` as a child). Since `AppRoute`'s own hook instance hasn't had its `onAuthStateChanged` fire yet on first render, `user` is transiently `null` there even though `RootRoute` already knows otherwise, causing `useCategories(user?.uid ?? "")` to briefly fire with an empty uid and throw an invalid-collection-reference Firestore error before self-healing on the next render. Fix: pass `user` down from `RootRoute` instead of calling `useAuth()` again in `AppRoute`.
